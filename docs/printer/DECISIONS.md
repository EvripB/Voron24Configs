...
5. Use twelve Filamentalist FV3 passive rewinders, one per commissioned gate,
   inside the actively heated filament enclosure. Keep
   `has_filament_buffer: 0`; the rewinders are spool-management devices, not a
   separate Happy Hare filament-storage buffer.
...
10. Use the Annex Belay hardware as Happy Hare's one-sided tension-feedback
    sensor. Do not install or enable Annex's standalone Belay Klipper module
    while Happy Hare owns synchronization and FlowGuard.

## Slicer and print start

11. Use OrcaSlicer, with separate printer profiles for TradRack and direct-feed
    operation. The direct-feed profile must not assume Happy Hare is enabled.
12. Preserve the complete validated TradRack start order recorded in
    `CURRENT.md`, including Happy Hare setup/check before heating and initial
    tool loading after `PRINT_START`.
13. Continue using the slicer purge tower for flushing until a purge
    bucket/Blobifier design is deliberately implemented and validated.
14. Treat maximum volumetric speed as a filament/hotend/nozzle combination.
...
15. Keep Klipper pressure advance at 0.02 for the current tested configuration.
    The 0.04 experiment caused corner/end gaps and is not a current default.
16. Retain 25% top/bottom infill-wall overlap for the profile where reducing it
    to 10% did not improve the observed roughness.
17. Do not repeat slow PLA printing with a 100 °C bed in a closed enclosure.
    Earlier tests ended in heat creep; use material-appropriate temperatures
    and ventilation.

## Maintenance and records

18. Separate host software updates from MCU flashing. Flash an MCU only for an
...
22. For the TradRack Orca profile, use Orca's explicit adaptive-bed-mesh bounds
...
23. Rebuild the host from the latest supported 64-bit MainsailOS rather than
...
24. Keep reproducible, credential-free custom host source in the public
...
25. Treat `BACKUP_CFG` as a configuration Git operation only. It is not a
...
26. Approach the front-left Crossbow cutter from in front of its depressor.
...
27. Depart the Crossbow cutter laterally before restoring a saved toolhead
...
28. Retain `flowguard_max_relief` at 80 mm. The two 80.19/80.20 mm compression
